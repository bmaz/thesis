...
as long as this sample is ``representative”. Why do we need representativity?


Assume that we get access to a subsample of the documents published on Twitter,
...
It seems very difficult to correct for this kind of biases. One can always control for a number of observable variables,
...
But for the interaction between social media and mainstream media, we need more than representativity, in a sense we need completeness.
E.g. for determining the news breaker, representativity is not enough, because we can miss some tweets before the first
news articles. It is why it is so important here not only to build a ``representative" sample of tweets, but to built
...
It should also be mentioned that the DLWeb does not guarantee to collect all tweets containing a given hashtag. Capturing on specific hashtags allows them to get a larger share of the tweets containing these hashtags than we do, but we also captured some tweets that they didn't collect.
