...
^2 + max(1, w))$.
 ...
 Our version of the algorithm also deals with ``empty" documents, i.e. documents that contain no words or only stop-words. This is frequently the case in datasets of tweets. Empty documents are not clustered, i.e. labeled as $-1$. Our version of the algorithm is described in Algorithm \ref{FSD mini}.
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{algorithm}
...
\begin{figure}[ht]
  ...
  \caption[SBERT architecture at inference]{SBERT architecture at inference, to compute similarity scores. This architecture is also used during part of the training with mean-square-error as regression objective function. This diagram is provided in the Sentence-BERT paper \cite{reimers_2019_sentence}.}
  \label{Fig:SBERT}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\caption[SVM classification results on the English corpus]{SVM classification results on the English corpus. Each cell indicates the mean and standard deviation of 5 runs (with different train/test splits), in percentages.} \label{classif_en}
\end{center}
\end{table}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\caption[SVM classification results on the French corpus]{SVM classification results on the French corpus. Each cell indicates the mean and standard deviation of 5 runs (with different train/test splits), in percentages.} \label{classif_fr}
\end{center}
\end{table}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\caption[FSD clustering results for each corpus.]{FSD clustering results for each corpus. Performance is assessed using the "Best Matching F1" score. For each model, the best $t$ threshold value was selected by successive tests. The batch size parameter $b$ is fixed to 8. The window-size parameter $w$ is fixed to the average number of tweets per day in each corpus.} \label{clustering}
\end{center}
\end{table}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{subtable}{1\textwidth}
    \resizebox{\textwidth}{!}{%
    \rowcolors{2}{white}{gray!25}
    \input{tables/DMM_DBSCAN_FSD_en}
    }
...
\begin{subtable}{1\textwidth}
    \resizebox{\textwidth}{!}{%
    \rowcolors{2}{white}{gray!25}
    \input{tables/DMM_DBSCAN_FSD_fr}
    }
